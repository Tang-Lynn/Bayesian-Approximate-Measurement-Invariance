...
# Arguments
# 1. lavout, the output of the two-group CFA model with 
...
BMI1 <- function(lavout,type,data,a,minBF,times,seed){
  #Part1: prepare for recomputing the model
  # get the estimates
  estiOut <- parameterEstimates(lavout)
  ...
  groupVariable <- lavInspect(lavout, what = "group")
  ...
  if (type == 'metric'){
    ...
    # run lavvan again
    lavAdjusted <- cfa(Model, data, std.lv = TRUE, group = groupVariable)
    ...
    if(is.nan(BF1)){
      ...
      while(partBF < minBF & length(PartEachBF) != 1){
        ...
        # run lavvan again 
        lavPart <- cfa(partAdjustedModel, data, std.lv = TRUE, group = groupVariable)
        ## get the adjusted loadings
        # get the adjusted estimates
        ...
        for(i in 1:indicatorNum){
          if(partLoadMaxG1[i]>partLoadMaxG2[i]){
            partLoadMax[i] <- a *partLoadMaxG2[i]
            
          }else{
            partLoadMax[i] <- a *partLoadMaxG1[i]
          }
        } 
        # get the upper bound 
      ...
      if(partBF >= minBF) {
        cat("\n With the minimum Bayes factor >=",minBF,",","partial approximate metric invariance is supproted. 
  ...
  # testing scalar invariance
    ...
    minIntG1 <-  psych::describe(dataG1,quant = c(.20,.80))[, c(14)]
    maxIntG1 <-  psych::describe(dataG1,quant = c(.20,.80))[, c(15)]
    difG1 <- maxIntG1 - minIntG1
    # robust max and min intercept in group 2 
    minIntG2 <-  psych::describe(dataG2,quant = c(.20,.80))[, c(14)]
    maxIntG2 <-  psych::describe(dataG2,quant = c(.20,.80))[, c(15)]
    difG2 <- maxIntG2 - minIntG2
    # get the bound
    dif <- 0
    for(i in 1:indicatorNum){
      if(difG1[i]>difG2[i]){
    ...
    if(is.nan(BF1)){
      ...
      while(partBF < minBF & length(PartEachBF) != 1){
        ...
        # run lavvan again 
        lavPart <- cfa(adjustedModel, data, std.lv = TRUE, group = groupVariable)
        ## get the adjusted intercepts
        # get the adjusted estimates
        estiPart <- parameterEstimates(lavPart)
        # get the adjusted intercepts
...
result1 <- BMI1(lavout,type = 'metric',data = sesamesim,a = 0.2,minBF = 10,times=2,seed=2020)
# partial approximate metric invariance
result2 <- BMI1(lavout,type = 'metric',data = sesamesim,a = 0.2,minBF = 65,times=2,seed=2020)
# approximate scalar invariance
result3 <- BMI1(lavout,type = 'scalar',data = sesamesim,a = 0.2,minBF = 3,times=2,seed=2020)
# partial approximate scalar invariance
result4 <- BMI1(lavout,type = 'scalar',data = sesamesim,a = 0.2,minBF = 10,times=2,seed=2020)
